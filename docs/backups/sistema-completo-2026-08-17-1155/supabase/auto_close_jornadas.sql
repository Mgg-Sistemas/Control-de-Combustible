-- ============================================================================
-- AUTO-CIERRE de jornadas (hora Caracas, UTC-4) — ACTUALIZADO 2026-08-14:
--   NOCHE cierra a las 7:00am del día siguiente (12h, 7pm→7am) para TODAS las máquinas.
--   Se QUITA el cierre a la 1:00am (antes noche = 6h): la noche ahora es permanencia de
--   hasta 12h. DÍA sigue cerrando a las 7:00pm. La regla del inspector SOS La Guaira
--   (12h fijas de DÍA) NO se toca — vive solo en el turno DÍA, más abajo.
--   EXCEPCIÓN 24H: el COMPRESOR CON MARTILLO (serial/placa '79669') trabaja
--   24 horas continuas → NUNCA se auto-cierra; su jornada se cierra a mano.
--   (Antes había un trato aparte para LUMINARIA que cerraba 7am; ahora que TODAS
--   cierran 7am, ese caso especial se eliminó.)
--
...
begin
  ...
  loop
    -- Fin del turno (hora Caracas): NOCHE -> 7:00am del día siguiente (12h, 7pm→7am,
    -- para TODAS las máquinas — regla cliente 14-ago-2026: se quita el cierre a la
    -- 1:00am); DÍA -> 7:00pm. El inspector SOS La Guaira / la regla del DÍA NO se tocan.
    ...
    -- 07:00 (noche) o 19:00 (día). Aunque alguien edite mal `end_ts`, si su hora no es
    -- 7 ni 19 se salta → JAMÁS a otra hora inesperada.
    ...
    if now() >= end_ts then
      if r.jornada_shift = 'night' then
        -- NOCHE: horas REALES (inicio → 7:00am = hasta 12h desde las 7pm). Requiere inicio
        -- ANTES del fin (si no, sumaría 0/negativo) — se deja para cierre manual.
