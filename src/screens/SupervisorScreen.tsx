...
export default function SupervisorScreen({ initialMachineId, onConsumed, onSistema }: { initialMachineId?: string; onConsumed?: () => void; onSistema?: () => void } = {}) {
  ...
  const [batchShift, setBatchShift] = useState<Shift>('day');
  ...
  const assignBatch = async (insp: { id: string; name: string }) => {
    if (batchBusy || pendSelected.size === 0) return;
    setBatchBusy(true); setNotice(null);
    const ids = Array.from(pendSelected);
    let ok = 0, fail = 0;
    for (const mid of ids) {
      const m = machines.find((x) => x.id === mid);
      const res = await assignInspector(mid, insp.id, insp.name, batchShift);
      if (res.error) { fail++; }
      else { ok++; if (m) logAudit('CHECK', 'machinery', mid, `${m.code} · ${shiftLabel(batchShift)} → ${insp.name} (lote)`); }
    }
    setBatchBusy(false);
    await reloadAssigns();
    setPendSelected(new Set());
    setBatchOpen(false);
    setNotice(`✅ Asignación por lotes: ${ok} máquina(s) OK${fail > 0 ? ` · ⚠️ ${fail} fallaron` : ''} · ${shiftIcon(batchShift)} ${shiftLabel(batchShift)} → ${insp.name}.`);
  };
    ...
    <Screen>
      ...
      <Modal visible={checkOpen} animationType="slide" onRequestClose={() => setCheckOpen(false)}>
        <Screen>
          ...
          {!checkInspector && pendOpen && batchOpen ? (
            // ── ASIGNACIÓN POR LOTES: elegir a qué inspector/turno van las N máquinas
            //    marcadas con checkbox en "Pendientes por asignar". ──────────────────
            <>
              <TouchableOpacity onPress={() => setBatchOpen(false)} style={{ alignSelf: 'flex-start', marginBottom: spacing.xs, borderWidth: 1, borderColor: colors.border, borderRadius: radius.pill, paddingHorizontal: spacing.sm, paddingVertical: 4 }}>
              ...
              <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.sm }}>
                {(['day', 'night'] as Shift[]).map((s) => (
                  <TouchableOpacity key={s} onPress={() => setBatchShift(s)} style={{ flex: 1, alignItems: 'center', paddingVertical: spacing.sm, borderRadius: radius.md, borderWidth: 1.5, borderColor: batchShift === s ? colors.primary : colors.border, backgroundColor: batchShift === s ? colors.primary : colors.surface }}>
                    <Text style={{ color: batchShift === s ? colors.primaryContrast : colors.text, fontWeight: '800', fontSize: 13 }}>{shiftIcon(s)} {shiftLabel(s)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <TextInput value={batchQuery} onChangeText={setBatchQuery} placeholder="🔎 Buscar inspector por nombre…" placeholderTextColor={colors.muted} style={input} />
              <ScrollView style={{ marginTop: spacing.xs }} keyboardShouldPersistTaps="handled">
                {inspectors.filter((p) => !batchQuery.trim() || norm(p.name).includes(norm(batchQuery.trim()))).map((p) => (
                  <TouchableOpacity key={p.id} disabled={batchBusy} onPress={() => assignBatch({ id: p.id, name: p.name })} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, marginBottom: spacing.xs, opacity: batchBusy ? 0.6 : 1 }}>
                    <Text style={{ fontSize: 20 }}>👮</Text>
                ...
                {batchBusy ? <Text style={{ color: colors.muted, fontSize: 12, textAlign: 'center', marginTop: spacing.sm }}>Asignando…</Text> : null}
            ...
            <>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.xs }}>
                ...
                {pendSelected.size > 0 ? (
                  <TouchableOpacity onPress={() => setBatchOpen(true)} style={{ flex: 1, alignItems: 'center', backgroundColor: colors.primary, borderRadius: radius.pill, paddingVertical: 6 }}>
