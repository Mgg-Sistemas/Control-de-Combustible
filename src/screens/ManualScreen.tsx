...
const SECTIONS: Sec[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '📍 Ubicaciones tácticas (botón en 📊 Conteo de equipos): genera el "INVENTARIO DE MAQUINARIA" en PDF, con el membrete oficial del Plan Venezuela Renace (ola tricolor y logotipo). El encabezado trae la Fecha (se llena sola) y dos líneas en blanco — Empresa y Responsable — para completar a mano al imprimirlo, porque el reporte cubre a LICCIONE y a GOLDEN TOUCH a la vez. Se descarga como "Reporte - Inventario de maquinaria". Cuenta el MISMO universo que el Catálogo (TODAS las máquinas menos las RETIRADAS). El reporte va 100% POR EMPRESA: arriba la cantidad de maquinaria por empresa y luego el listado de máquinas agrupado por EMPRESA en dos grupos: LICCIONE (sus máquinas) y GOLDEN TOUCH (las de Golden + TODAS las demás empresas), cada una con su MARCA y MODELO en columna propia (CAT 320, Komatsu PC200…; si sale un guión es que esa máquina no los tiene cargados en su ficha del Catálogo, no es falla del reporte), su PLACA / SERIAL en columna propia, su ubicación real (referencia + sector Este/Oeste y subzona por GPS: Macuto, Caraballeda, Aeropuerto…) y estado (Operativo / Inoperativo / En espera). Las máquinas sin ubicación cargada salen como "Desplegadas por todo el territorio de La Guaira". Cierra con un conteo por clasificación. (Ya NO trae los resúmenes por zona Este/Oeste.) OJO: tipo, marca y modelo son TRES campos distintos de la ficha — el tipo es qué clase de equipo es (excavadora, volteo), la marca de quién es (CAT, Komatsu) y el modelo cuál (320, PC200). Antes el reporte imprimía el TIPO haciéndolo pasar por marca; desde el 31/08/2026 imprime la marca y el modelo de verdad, y el tipo queda en gris junto al código.' },
