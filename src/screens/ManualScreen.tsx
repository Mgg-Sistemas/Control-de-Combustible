...
const SECTIONS: Sec[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'p', text: 'Vista del supervisor (teléfono): arriba tiene una fila de tarjetas resumen de SUS máquinas (m³ removidos hoy, edificios, máquinas asignadas, trabajando, averiadas, m³ totales); al tocar "Trabajando" o "Averiadas" se filtra la lista. Por cada máquina puede registrar visita (GPS), iniciar/finalizar jornada, marcar avería o parada, actualizar la ubicación, registrar los m³ removidos y generar su reporte diario.' },
      { t: 'p', text: 'Registrar m³ removidos: en el detalle de la máquina escribe la cantidad y toca "➕ Sumar m³" (se van sumando durante el día, viaje por viaje) o "✏️ Fijar total" para corregir el total del día. "m³ removidos hoy" es lo del día; "m³ totales" se va acumulando día a día.' },
      { t: 'p', text: 'Reporte de Actividades (botón "📋 Reporte del día"): cada supervisora llena su reporte diario — N° de reporte, M³ removidos controlada, M³ acarreo de vestigios, Cuerpos siniestrados, Traslado camión — y una observación del día. Puede Guardar o Descargar PDF. Los totales "desde el inicio" (m³ y cuerpos acumulados) los calcula el sistema: base registrada + todos los reportes. El admin ajusta esa base en el panel con "⚙️ Editar base" (m³ base, cuerpos base y fecha de corte).' },
      { t: 'p', text: 'Panel de Obras Públicas (Más → 🏛️ Obras Públicas): panel de admin/coordinador que AGREGA todo el módulo (todos los supervisores). Muestra KPIs (máquinas asignadas, trabajando ahora, averiadas/paradas, m³ del día, edificios), el Reporte de Actividades consolidado del día (m³ removidos, m³ acarreo, cuerpos, traslado camión) con los acumulados desde el inicio, gráficos (acarreo total por día, distribución por estado 7/30), el estado de la flota en campo con sus m³ removidos (y su supervisor) y la tabla de registros de acarreo. Puedes filtrar por supervisor con los chips de arriba, y tocar un KPI para filtrar la flota. El admin ajusta la base acumulada con "⚙️ Editar base".' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🟢 Inspector SOS LA GUAIRA — máquinas siempre trabajando: las máquinas asignadas al inspector SOS LA GUAIRA nunca se muestran como parada ni averiada; siempre cuentan como trabajando y sus horas paradas se cuentan como trabajadas (en el catálogo, el panel de Inspecciones, el teléfono y todos los reportes). Si se les reporta una avería, el ticket sí queda en Mantenimiento de Maquinaria para el mecánico, pero no cambia su estado de trabajando.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🕒 Cierre de jornada: el DÍA cierra automático a las 7:00pm y la NOCHE a la 1:00am (la máquina que quede abierta la cierra el sistema). El único equipo que NO se cierra (trabaja 24h) es el COMPRESOR CON MARTILLO (serial 79669). El inspector puede FINALIZAR manualmente antes; si lo hace ANTES de la hora de cierre (día <7pm / noche <1am) el sistema le PIDE OBLIGATORIO el MOTIVO del cierre. Excepción: el inspector "SOS LA GUAIRA" (máquinas siempre activas) cierra/finaliza automático y NO pide motivo.' },
      ...
      { t: 'note', text: 'Avería de maquinaria desde el check-in: en la ventana "✅ Revisé la máquina", el inspector puede abrir "🛠️ Avería de maquinaria", elegir el material (caucho/aceite/filtro/repuesto) con su cantidad y nota, o tocar ✏️ Otro para describir a mano una falla distinta (ej. no arranca, fuga de aceite), y "Registrar avería". Es la misma función que el operador y cae en el módulo de Mantenimiento de Maquinaria como solicitud pendiente.' },
  ...
  },
  {
    ...
    blocks: [
      { t: 'p', text: 'Módulo para los coordinadores de mantenimiento. Tiene cinco pestañas: ⏳ Averías (lo que reportan por QR, por empresa → máquina), 🔧 En reparación, ✓ Historial, ⏱️ Horómetros y 📊 Reporte.' },
