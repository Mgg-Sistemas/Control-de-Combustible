...
const SECTIONS: Sec[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '⚠️ ARREGLADO EL 27/08/2026 — “la contraseña es correcta y no me deja entrar”. Hasta esa fecha, al pulsar el ojo 👁️ para revisar la clave, la pantalla la mostraba TODA EN MAYÚSCULA aunque la clave real tuviera minúsculas: lo que se veía y lo que se guardaba no eran lo mismo. El administrador creaba “Sos2026”, pulsaba el ojo, leía “SOS2026”, y eso dictaba; el trabajador escribía SOS2026 y el sistema lo rechazaba. Parecía intermitente porque solo pasaba si alguien pulsaba el ojo Y la clave tenía minúsculas. Ya no pasa.' },
      { t: 'note', text: '🔤 LAS CONTRASEÑAS VAN SIEMPRE EN MAYÚSCULA (27/08/2026). Escribas como escribas, la clave se convierte sola a MAYÚSCULA: al crear un usuario, al cambiar la clave y al entrar. Si tecleas “sos2026”, se guarda y se manda “SOS2026”. Toda la app se escribe en mayúscula y las contraseñas eran la única excepción — y esa excepción fue la que causó el lío. Ahora lo que ves es exactamente lo que se guarda. En la práctica: ya no importa si la escribes con mayúsculas o sin ellas, ni si el trabajador la teclea distinto de como se la dictaste; entra igual. ✅ A nadie se le queda la clave vieja sin funcionar: las guardadas antes del 27/08 siguen sirviendo tal cual, porque el sistema prueba primero la mayúscula y si no es, reintenta con lo tecleado — y ese reintento NO gasta uno de los 3 intentos antes del bloqueo. El único caso que sigue sin funcionar es una clave vieja escrita toda en minúscula que se teclee toda en mayúscula: si le pasa a alguien, restablece su clave y queda con la regla nueva.' },
      { t: 'note', text: 'Si alguien sigue sin entrar, revisa que no esté BLOQUEADO por 3 intentos fallidos.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🏢 ELEGIR QUÉ EMPRESAS SALEN (31/08/2026). El reporte de Ubicaciones tácticas ahora respeta las EMPRESAS MARCADAS más abajo en la misma pantalla ("Empresas (marca una o varias)"): marcas las que quieras y el PDF sale solo con esas. Sin marcar ninguna sale la flota completa, como siempre. Justo encima del botón hay un renglón que te dice qué va a salir antes de tocarlo ("Sale dividido por empresa, solo con: …"), para que no haya que adivinar. El alcance queda IMPRESO en el encabezado del PDF y también en el nombre del archivo — dos reportes de empresas distintas ya no se llaman igual ni se ven iguales en papel. Vale igual para la versión 🎭 SIMULADO.' },
      { t: 'note', text: '📍 Ubicaciones tácticas (botón en 📊 Conteo de equipos): genera el "INVENTARIO DE MAQUINARIA" en PDF, con el membrete oficial del Plan Venezuela Renace (ola tricolor y logotipo). El encabezado trae la Fecha (se llena sola) y dos líneas en blanco — Empresa y Responsable — para completar a mano al imprimirlo, porque el reporte puede cubrir varias empresas a la vez. Se descarga como "Reporte - Inventario de maquinaria" MÁS los nombres de las empresas elegidas, para que dos reportes distintos no se pisen en la carpeta de descargas. Cuenta el MISMO universo que el Catálogo (TODAS las máquinas menos las RETIRADAS). El reporte va 100% POR EMPRESA: arriba la cantidad de maquinaria por empresa y luego el listado agrupado por EMPRESA — UNA SECCIÓN POR CADA EMPRESA DE VERDAD (31/08/2026). Antes metía todo en dos cubetas fijas, LICCIONE y GOLDEN TOUCH, y esa segunda se tragaba a TODAS las demás empresas: una máquina de Savanna o de Ferreconstrucciones salía impresa bajo el nombre de otra empresa. Las que no tienen empresa cargada caen en "Sin empresa", al final, sin inventarles ninguna. Cada máquina sale con su PLACA / SERIAL en columna propia, su ubicación real (referencia + sector Este/Oeste y subzona por GPS: Macuto, Caraballeda, Aeropuerto…) y estado (Operativo / Inoperativo / En espera). Las máquinas sin ubicación cargada salen como "Desplegadas por todo el territorio de La Guaira". Cierra con un conteo por clasificación. (Ya NO trae los resúmenes por zona Este/Oeste.)' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🏢 CARBOZULIA Y SEGURIDAD TIENEN SU PROPIA PESTAÑA (29-ago-2026). En la línea de Estado hay dos chips nuevos al final: "Carbozulia" y "Seguridad". Esa gente YA NO SE MEZCLA con el resto: no aparece ni se cuenta en Todos, Activos, Inactivos ni Otro — para verla hay que tocar su chip. La separación sale de lo que YA está en la ficha, no hay campo nuevo que llenar: Carbozulia = los que tienen esa EMPRESA FILTRO NÓMINA; Seguridad = los que tienen SEGURIDAD como CARGO. Si alguien estuviera en los dos (hoy no pasa), manda Carbozulia, para que nadie salga contado dos veces. Cómo cambió el conteo: "Todos" pasó de 338 a 282 y "Activos" de 237 a 181; Inactivos (61) y Otro (40) quedan igual; los que salieron están en Carbozulia · 40 y Seguridad · 16 (282 + 40 + 16 = 338, la plantilla completa). Para mover a alguien de pestaña se le cambia la ficha: ✎ Editar → "🏢 Empresa filtro nómina" o el campo Cargo; el chip se recalcula solo. El 📊 Reporte respeta la pestaña que tengas puesta.' },
      ...
      { t: 'note', text: '🏢 PESTAÑAS CARBOZULIA Y SEGURIDAD EN EL PERÍODO (29-ago-2026). Junto a Activos, Todos e Inactivos/Desincorporados hay dos pestañas más: Carbozulia y Seguridad, para ver y exportar a Excel a esa gente aparte sin buscarlos uno por uno. Se reconocen por lo que ya está en la ficha, igual que en Empleados: Carbozulia por su "Empresa filtro nómina" y Seguridad por el cargo SEGURIDAD. ⚠️ OJO, una diferencia A PROPÓSITO frente a Empleados: allá esa gente SALE de "Todos" y "Activos"; AQUÍ NO — siguen contando en las tres pestañas de siempre y las dos nuevas son solo un atajo para filtrarlos. El motivo es que TODOS los períodos mezclan a los tres grupos (ej. "Carbozulia semana 4 de agosto" son 22 de Carbozulia + 16 de Seguridad + 181 del resto) y el TOTAL DEL PERÍODO de arriba los incluye a todos: si se escondieran, la lista dejaría de cuadrar con el monto y parecería un error de pago. El nombre del período es solo una etiqueta: que se llame "Carbozulia" no significa que adentro solo haya gente de Carbozulia.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'steps', items: [
        ...
        'Marca Desayuno, Almuerzo, Lunch o Cena (4 comidas del día): cada botón se marca 1 sola vez por día por persona.',
        'Queda guardado con la hora. Debajo se ve lo ya marcado hoy a esa persona.',
      ] },
      { t: 'note', text: 'Arriba de la pantalla de Cocina hay 4 tarjetas de conteo del día (🌅 Desayuno · 🍽️ Almuerzo · 🥪 Lunch · 🌙 Cena) que muestran cuántas se han repartido HOY. Suben SOLAS en tiempo real: apenas escaneas un desayuno, la tarjeta de Desayuno sube +1 (aunque reparta otra persona desde otro teléfono).' },
      ...
      { t: 'p', text: 'Módulo "Distribución de comida" (en Más, para el jefe): por día muestra 4 tarjetas de conteo por comida (🌅 Desayuno · 🍽️ Almuerzo · 🥪 Lunch · 🌙 Cena, empresa + persona) que suben en tiempo real, y debajo el detalle repartido POR EMPRESA y por persona, con sus totales. Usa las flechas ◀ ▶ para cambiar de día.' },
      ...
      { t: 'steps', items: [
        ...
        'Toca uno de los 4 botones grandes: Desayuno, Almuerzo, Lunch o Cena (cada uno 1 sola vez por día por empresa).',
        'El sistema sugiere el total = máquinas de la empresa × 2 + 15; el cocinero escribe cuántas comidas entregó realmente y registra.',
      ] },
      { t: 'note', text: 'Cada comida (desayuno/almuerzo/lunch/cena) se puede marcar UNA sola vez por día por empresa. Queda guardado con la empresa, la cantidad, la hora y quién la registró. Ese registro ES el control de asistencia/entrega de la empresa.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🛒 Compras directas (reemplaza a "Solicitudes de pedido"): para una compra YA HECHA que quieres cargar de una vez. En Compras → "🛒 Compras directas" toca "+ Nueva", elige la EMPRESA y el PROVEEDOR, agrega los renglones (producto · cantidad · PRECIO), adjunta la FACTURA (imagen o PDF) y toca "Registrar compra directa". Al guardarla: (1) cada renglón ENTRA al inventario con su precio (recalcula el costo promedio) y (2) le queda su código correlativo (CD-0001, CD-0002…). NO genera cuenta por pagar: solo carga la factura y pasa al inventario. En la lista, cada compra trae "📎 Ver factura" para revisar la factura cargada. A diferencia del Requerimiento (que se pide y el jefe aprueba antes de comprar), la compra directa es inmediata: no pasa por aprobación. ✏️ Editar una compra ya cargada: si te equivocaste en un precio o una cantidad, entra a la compra y corrígela; al guardar, el inventario se rehace solo (se retira la entrada anterior y se vuelve a cargar con los datos nuevos), así que el costo promedio queda bien y no hace falta ningún ajuste a mano. Requiere correr, EN ESTE ORDEN: supabase/compras_directas.sql → supabase/compras_directas_editar.sql → supabase/compras_directas_sin_cuenta.sql (este último es el que quita la cuenta por pagar, y va de último porque reemplaza funciones de los otros dos).' },
      ...
      { t: 'p', text: 'Tipo de producto y filtro: al crear/editar un producto puedes ponerle un TIPO (bombona, silla, mecate…). Escríbelo o tócalo de las sugerencias. El tipo sale en el reporte de productos y en 🔁 Movimientos, donde sí puedes filtrar por él. ⚠️ YA NO hay chips "Filtrar por tipo" encima de la lista de productos (se quitaron el 23/08/2026): si los buscas, no es que te falten permisos, no existen. Para acotar la lista usa el buscador; para filtrar por tipo, la pestaña 🔁 Movimientos.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '⚡ LA VISTA PREVIA YA NO SE QUEDA PENSANDO (26-ago-2026): al tocar 📄 Exportar, la ventana de vista previa aparece DE UNA VEZ diciendo «Preparando la vista previa…» mientras arma el documento — y se puede cancelar mientras tanto. Antes el navegador armaba TODO el reporte antes de dibujar nada, así que no se veía ni la ventana y parecía que la aplicación se había colgado. Además las fotos ya no frenan el armado de las hojas, el reporte pide los expedientes viejos ya recortados por las fechas del filtro (antes se traía el histórico completo), y las dos consultas salen a la vez. Y al tocar Imprimir, si alguna foto todavía viene bajando, la ventana dice «Cargando las fotos…» y espera — antes se mandaba a imprimir de una y las hojas salían con el recuadro vacío o negro. Si una foto se queda pegada, a los 15 segundos imprime igual con lo que haya: nunca te deja trancado. De paso se arregló un error viejo: con más de 1.000 expedientes viejos la consulta se cortaba en silencio y el PDF salía incompleto. EL DOCUMENTO QUE SALE ES EXACTAMENTE EL MISMO: solo cambió cuánto tarda en aparecer.' },
      { t: 'note', text: '✏️ EDITAR UN SERVICIO YA REGISTRADO (26-ago-2026): en 🧾 Servicios, cada servicio tiene el botón "✏️ Editar" (solo con permiso de escritura). Abre el MISMO formulario con todo cargado —fecha, máquina, quién lo hizo, tipos de intervención, problema, acciones, fotos y repuestos— y se guarda con "💾 Guardar cambios". Editar NO toca la máquina ni la avería: la frontera del módulo no se movió.' },
      { t: 'note', text: '🕓 Y QUEDA CONSTANCIA DE QUIÉN LO TOCÓ: debajo del servicio sale la línea "✏️ Última edición: Fulano · 26/08/2026 3:15 p.m.", y junto a los botones aparece "🕓 Ver cambios", que abre la lista completa de ediciones —quién, cuándo y qué campo cambió, con el valor de antes en rojo y el de después en verde—. La bitácora SOLO SE AGREGA: ni el que editó ni nadie más la puede modificar ni borrar. Los servicios que nunca se editaron no muestran ninguna de las dos cosas. "Ver cambios" lo puede abrir cualquiera que entre a la pestaña, tenga o no permiso de escribir: saber quién cambió un registro es justo lo que necesita el que solo mira.' },
      { t: 'note', text: '⚠️ Detalles de la edición: si no cambiaste nada y le das a guardar, no se anota nada (te avisa, en vez de llenar la bitácora de ediciones vacías). Editar NO le cambia el dueño al registro: quien lo REGISTRÓ sigue siendo el mismo para siempre; lo que se guarda aparte es quién lo EDITÓ. Hay que correr UNA SOLA VEZ «supabase/servicio_editar.sql» en Supabase → SQL Editor: mientras nadie lo corra, editar funciona igual y el servicio se guarda bien, pero no queda el rastro — y la pantalla te lo dice nombrando el archivo, en vez de fingir que lo guardó. Los administradores con permiso de Auditoría ven además estas ediciones —y los borrados— en la pantalla de 🕵️ Auditoría.' },
      ...
      { t: 'note', text: '📄 PDF de ficha técnica y reparaciones: en 🧾 Servicios filtras por máquina y por rango de fechas, y tocas "📄 Exportar PDF". Si filtraste UNA SOLA máquina, la primera página sale con su FICHA TÉCNICA (foto, tipo de equipo, marca, modelo, serial, placa, identificador, empresa y encargado) y de la segunda en adelante van todas sus reparaciones. Si filtraste varias máquinas, sale agrupado por máquina y sin ficha — cuarenta fichas seguidas no le sirven a nadie. Las reparaciones viejas, de antes de esta pestaña, también salen, marcadas como "Registro anterior" porque traen menos datos.' },
      { t: 'note', text: '📄 Sacar UNA SOLA hoja (25-ago-2026, pedido del taller): antes «Exportar PDF» sacaba TODO lo del filtro, y como «Desde» y «Hasta» arrancan vacíos, quien registraba un servicio de prueba recibía un PDF con ese, con el que ya tenía montado y con los expedientes viejos del taller, todo junto. Ahora cada servicio de la lista tiene su botón «📄 Solo esta hoja»: saca ESE y nada más, sin ficha técnica (la hoja ya lleva la foto, el equipo y el serial) y sin arrastrar ningún otro registro. Cabe en una página mientras el servicio no lleve más de tres repuestos; con más, sigue en la segunda.' },
      { t: 'note', text: '📅 Para sacar el DÍA: arriba de «Exportar» hay tres atajos — «📅 Hoy», «📅 Ayer» y «✕ Sin fecha» — que ponen las dos fechas de un toque. Y la casilla «🧰 Traer también los expedientes viejos del taller» se puede apagar: viene encendida (es como funcionó siempre), pero apagada el PDF trae SOLO los servicios registrados en esta pestaña, sin los marcados como «Registro anterior».' },
      { t: 'note', text: '👁️ El botón dice cuánto va a sacar ANTES de tocarlo: «📄 Exportar 3 servicios», o «+ viejos» con la casilla encendida. Cuenta SERVICIOS, nunca páginas, y es a propósito: si el filtro deja UNA sola máquina el PDF empieza con su ficha técnica en página aparte (o sea que 3 servicios son 4 páginas), y una hoja con muchos repuestos ocupa dos. Prometer un número de páginas y entregar otro sería repetir la misma sorpresa.' },
      { t: 'note', text: '📋 Cada reparación sale como la HOJA de papel (25-ago-2026): el PDF se ajustó al formato del "REPORTE DE MANTENIMIENTO / REPARACIÓN — MAQUINARIA PESADA" que llena el taller. Cada reparación es una hoja completa: franja azul con el título, la FOTO de la máquina, los cuatro renglones de cabecera (Fecha · Operador / Técnico · Equipo (ID / Modelo) · Código de Serial), las CASILLAS de tipo de intervención, y los tres recuadros con sus bandas (DESCRIPCIÓN DEL PROBLEMA · ACCIONES REALIZADAS · REPUESTOS UTILIZADOS). Antes cada reparación salía como una tarjetita resumida; ahora el PDF y el papel se pueden poner uno al lado del otro.' },
      { t: 'note', text: '☑️ Las casillas salen TODAS, marcadas y sin marcar — eso es lo que las hace casillas y no etiquetas. Son las mismas del formulario en pantalla y en el mismo orden, así que si agregas un tipo nuevo (ej. Soldadura) aparece también en el PDF. Un tipo que se marcó y DESPUÉS desactivaron igual sale, al final de la fila y marcado: un servicio viejo no puede perder lo que dijo.' },
      { t: 'note', text: '✍️ Las firmas ahora van al pie de CADA hoja, no una sola vez al final del documento: cada intervención la firman el técnico que la hizo y su supervisor, igual que en el papel. Cada reparación ocupa su propia página (una hoja vacía ya mide dos tercios de la carta, así que dos nunca caben juntas); si lleva muchos repuestos se pasa de largo y se parte, pero las dos rayas de firmar no se quedan solas en una página en blanco. Los recuadros se imprimen aunque estén vacíos, para poder llenarlos a mano.' },
      { t: 'note', text: '🔎 Una referencia larga sin espacios no se pierde: si alguien pega un código de repuesto larguísimo sin separaciones, el texto se corta y sigue en la línea de abajo en vez de salirse del recuadro. Antes de este arreglo lo que se salía NO llegaba al papel — no se veía mal, desaparecía.' },
      { t: 'note', text: '🚜 La foto de la máquina va en CADA hoja, no solo en la ficha. Cuando se imprime un rango con varias máquinas y después alguien separa los papeles, una hoja suelta tiene que poder decir de qué máquina es — por eso lleva además el Equipo con su placa/serial y el Código de Serial.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🔎 TOTAL POR EQUIPO — BUSCAR UNA MÁQUINA (31/08/2026, dentro de 🔧 JORNADA). Genera el informe por jornada y, en la vista previa, debajo del botón de descargar, hay un bloque nuevo "🔎 Total por equipo". Escribes el nombre, la PLACA, el SERIAL, el identificador, la empresa o el encargado, y salen las máquinas que coinciden; tocas las que quieras y arriba te da el TOTAL DE HORAS y el TOTAL EN DINERO de esos equipos, más el desglose ☀️ día / 🌙 noche y cuántas jornadas. Si no marcas ninguna, suma toda la flota del informe. Cada máquina se puede tocar para ver su detalle DÍA POR DÍA. Y hay un botón "⬇️ PDF por equipo" que imprime exactamente eso.' },
      { t: 'note', text: '📆 EL RANGO O LOS DÍAS QUE QUIERAS. El bloque trae dos modos: "📅 Todo el rango" (el mismo del informe) o "📆 Días sueltos", que muestra una pastilla por cada día con horas y te deja marcar los que quieras — el martes y el jueves solamente, por ejemplo. Si no marcas ninguno, te lo dice en vez de mostrarte un cero que parece un resultado.' },
      { t: 'note', text: '🛡️ ESTO NO CAMBIA NI UN NÚMERO DEL INFORME. Es la MISMA cuenta vista por equipo: buscar o marcar máquinas no recalcula nada ni saca máquinas del informe de arriba, y está fijado con prueba automática. Tres cosas que conviene saber: (1) el MONTO se suma DÍA POR DÍA — si a una máquina le cambiaron el precio a mitad de semana, cada jornada vale lo que valía ESE día, no el último; (2) si alguna jornada no tenía precio, la máquina lo avisa ("el monto está incompleto") en vez de mostrarte un total que parece completo; (3) el bloque NO muestra abonos ni saldo por pagar, porque esos son de la EMPRESA y no de la máquina — ponerlos al lado de las horas de un equipo daría una cifra de cobro inventada. Si eliges "🕒 Solo horas" en Contenido del PDF, el bloque también esconde el dinero.' },
      { t: 'note', text: '🔴 LAS QUE NO TRABAJARON TAMBIÉN SALEN. Una máquina averiada, parada o esperando instrucciones aparece en la lista con 0 horas, $0 y su motivo — para que buscarla dé un resultado y no parezca que no existe. No suma a los totales ni cuenta como equipo que trabajó.' },
      { t: 'bullets', items: [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '⚠️ ARREGLADO EL 27/08/2026 — “elijo un rol al crear y me lo deja en otro”. Hasta esa fecha, al CREAR un usuario el rol elegido podía perderse: quedaba como conductor y abría en la pantalla de Surtir ⛽. Entrando otra vez al usuario y cambiándole el rol sí funcionaba, y por eso parecía un capricho. La causa: crear y editar seguían caminos distintos, y el de crear tenía una lista de roles vieja que cambiaba por “conductor” cualquier rol que no reconociera, sin avisar (le pasó a Coordinador de Patio y luego a Coordinador de Inspectores). Ya no pasa: el rol que eliges es el que queda, y si no se pudiera fijar la pantalla te lo dice, avisando que el usuario SÍ se creó. Requiere haber corrido supabase/rol_coordinador_inspectores_enum.sql.' },
      { t: 'bullets', items: [
        'Cada usuario tiene UN rol: Administrador, Inspector, Analista, Operador, Conductor, Cocina, Coordinador de Patio o Coordinador de Inspectores — o uno de los roles personalizados creados en 🏷️ Roles del sistema. ⚠️ En el desplegable busca INSPECTOR, no "Supervisor": por dentro la clave sigue siendo supervisor, pero en pantalla siempre se lee inspector.',
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🔎 BUSCADOR Y PLACAS EN LOS FILTROS (31/08/2026, lista completa). Dos cosas que iban juntas. (1) LA PLACA EN CADA PASTILLA: en la fila CAMIÓN, cada pastilla decía solo el nombre — y como todos los camiones de la flota se llaman igual, "CAMION VOLTEO TORONTO", salían treinta pastillas idénticas y no había forma de saber cuál tocar. Ahora dicen "CAMION VOLTEO TORONTO · A28BC1J": el nombre y la PLACA; si el camión no tiene placa cargada, sale su SERIAL; si no tiene ninguno de los dos, queda solo el nombre. Es la misma placa que ya salía en el resumen y en el PDF, sacada del mismo lugar, así que los tres siempre dicen lo mismo. (2) EL BUSCADOR: arriba de los filtros hay una caja "🔎 Buscar camión por placa, listero o empresa". Escribe un pedazo de la placa (o del nombre del listero, o de la empresa) y las pastillas se reducen a las que coinciden. No distingue mayúsculas ni acentos. Debajo dice cuántas quedaron ocultas, y "✕ Limpiar" las devuelve todas.' },
      { t: 'note', text: '⚠️ EL BUSCADOR NO ESCONDE LO QUE TENGAS MARCADO. Un filtro marcado sigue a la vista aunque no coincida con lo que escribiste. Es a propósito: una pastilla marcada sigue filtrando la lista de abajo, y si se escondiera, la pantalla mostraría pocos viajes sin nada que explique por qué — el mismo problema que ya se corrigió el 22/08/2026. El buscador solo cambia QUÉ VES, nunca qué está filtrando: para dejar de filtrar hay que desmarcar la pastilla o tocar "✕ Limpiar filtros", que es otro botón.' },
      { t: 'note', text: '✍️ CUADRAR UN DÍA: PONER Y QUITAR VIAJES A MANO (31/08/2026, solo FULL CONTROL). Quien tenga nivel "Full control" en este módulo ya puede arreglar cualquier dato de cualquier camión, para el día que sea. AGREGAR: en el panel de la jefa hay una tarjeta "✍️ Cargar viajes a mano" — buscas el camión (por código, placa o serial), eliges EL DÍA, la hora del primero, CUÁNTOS VIAJES (hasta 30 por vez), el chofer y a nombre de qué listero quedan. Si cargas más de uno, se separan 5 minutos a partir de esa hora, para que después le puedas corregir la hora a cada uno por separado; apilarlos todos en el mismo minuto los volvería indistinguibles. QUITAR: en la lista completa, filtras el día y le das "🗑️ Borrar" a los que sobren, como siempre.' },
      { t: 'note', text: '✏️ EDITAR UN VIAJE COMPLETO (31/08/2026, solo FULL CONTROL). El botón de la lista completa ahora dice "✏️ Editar" (antes "Editar hora") y abre CUATRO campos: la FECHA (nueva — antes el día del viaje no se podía cambiar de ninguna manera), la HORA, el CHOFER / RESPONSABLE, y LO REGISTRÓ, que reasigna el viaje a otro listero. Cambiar de listero pregunta antes, porque el viaje deja de contar para uno y pasa a contar para el otro en el resumen por listero. Al listero de campo NO le cambia nada: él sigue viendo solo la hora, y solo de sus viajes de su jornada. Lo que NO se puede cambiar es el CAMIÓN: cambiarle el camión a un viaje no es corregirlo, es otro viaje — para eso se borra este y se carga el bueno, y así la auditoría conserva las dos cosas por separado.' },
      { t: 'note', text: '🏷️ LOS VIAJES CARGADOS A MANO SE DISTINGUEN. Todo viaje que se cargue desde la oficina sale con la etiqueta "✍️ cargado a mano" y guarda el nombre de quién lo cargó. Es a propósito: el reporte tiene que poder responder "¿esto lo contó alguien en el patio, o lo cuadraron después?", y sin la marca las dos cosas se verían idénticas. NO SE PUEDEN CARGAR VIAJES A FUTURO: solo de hoy hacia atrás, porque un viaje con fecha de mañana se contaría como trabajo hecho sin serlo. Todo esto queda además en la Auditoría (quién, qué y cuándo), igual que borrar.' },
      ...
      { t: 'note', text: '🟢 EL ESTADO DEL CAMIÓN YA NO IMPIDE REGISTRAR (31/08/2026). En ESTE módulo y solo en este, no importa cómo figure el camión: AVERIADO, PARADO o EN ESPERA de instrucciones, todos salen en el buscador y todos aceptan viajes. La ÚNICA excepción son las RETIRADAS, que siguen sin salir (ver la nota de abajo). Y ya no pregunta nada antes de guardar: donde antes salía "este camión figura AVERIADA, ¿de todas formas quieres registrar el viaje?", ahora registra y ya. El motivo: un viaje es algo que el listero VIO —el camión entró— mientras que el estado es una anotación de otro módulo que puede estar vieja o mal puesta; cuando las dos cosas se contradicen, gana lo que se vio. Antes, un camión marcado por error se quedaba sin manera de anotarle los viajes, y esos viajes se perdían. ESTO REEMPLAZA en parte la regla del 18/08/2026, que sacaba de la lista tanto las retiradas como las que estaban en espera: las en espera vuelven, las retiradas no.' },
      { t: 'note', text: '⬛ LAS RETIRADAS SIGUEN FUERA, Y ESTO ES A PROPÓSITO (31/08/2026, el mismo día). Al quitar el filtro entero, la lista del listero pasó de 61 a 89 camiones: los 28 nuevos eran RETIRADOS, con motivos cargados como "Fin de contrato" o "reemplazo por A74AB3P". Es decir, camiones que ya NO ESTÁN en la obra — y dos de ellos, sin placa ni serial, salían como dos renglones idénticos imposibles de distinguir. Una máquina AVERIADA o PARADA sigue en el patio y su estado es una anotación que puede estar vieja o mal puesta, así que contra un viaje que el listero VIO, gana lo que se vio. Una RETIRADA es otra cosa: es un hecho administrativo, el camión se fue, y no hay viaje observado que pueda contradecirlo. Si de verdad hubo un viaje de un camión ya retirado, la JEFA lo carga desde "✍️ Cargar viajes a mano", que sí tiene el catálogo completo: la puerta no se cierra, se mueve a quien puede juzgarlo.' },
      { t: 'note', text: '👤 DOS ARREGLOS DEL CHOFER (31/08/2026). (1) La consulta del chofer del turno ya NO puede frenar el registro: iba con espera abierta y ANTES de mirar si había señal, así que con el wifi del patio —que da señal sin internet— el botón se quedaba en "Registrando…" y, si el listero cerraba la app, EL VIAJE SE PERDÍA ENTERO sin haberse guardado siquiera en el teléfono. Ahora tiene tope de 4 segundos: si no contesta, se sigue con el chofer que ya se tenía. Un chofer viejo es un dato imperfecto; un viaje perdido no se recupera. (2) El botón de registrar ahora está DESHABILITADO mientras el chofer carga, y dice "Buscando el chofer…". Antes se podía tocar de una y el viaje quedaba con el chofer VACÍO, y después la pantalla pintaba el nombre como si todo hubiera salido bien — es la explicación más probable de los viajes sin chofer en el reporte.' },
      { t: 'note', text: '👁️ PERO EL ESTADO SE SIGUE VIENDO, Y SE GUARDA. Quitar el bloqueo no es esconder el dato: en el buscador cada camión sigue mostrando su estado con su color (✅ Operativa, 🔴 Averiada, 🟡 Parada, ⬛ Retirada, ⏳ Esperando instrucciones), y el camión escogido lo muestra antes de tocar el botón. Además el estado queda CONGELADO en el viaje, así que la jefa puede revisar después cuáles se registraron contra un camión averiado. Y lo más importante: registrar un viaje NO le cambia el estado a la máquina — este módulo no escribe nada en el catálogo de maquinaria, ni le abre jornadas, ni le toca el horómetro. La alerta de "camión sin viaje reciente" sí sigue sin reclamar por los averiados y retirados, que es lo correcto: una máquina retirada legítimamente no viaja.' },
      { t: 'note', text: '☀️🌙 VER LOS VIAJES POR TURNO (23/08/2026, panel de la jefa). En la LISTA COMPLETA hay una fila nueva TURNO, arriba de las de listero/empresa/camión, con dos pastillas: "☀️ Día" y "🌙 Noche", cada una con su cantidad. Día es de 7am a 7pm y noche de 7pm a 7am — el mismo corte con el que el sistema arma la jornada. Sin marcar nada salen los dos. Marcar un turno BAJA las cuentas de los otros filtros: con "🌙 Noche" marcado, los chips de listero muestran solo a quienes trabajaron de noche y con cuántos viajes, y el listero que no trabajó de noche desaparece de la fila (es lo que la hace útil). En el RESUMIDO, cada empresa/listero y cada camión traen su desglose "☀️ X · 🌙 Y", y el PDF gana dos columnas, ☀️ Día y 🌙 Noche, además del total. En el RESUMEN DEL DÍA, debajo de cada camión dice si anda "☀️ día", "🌙 noche" o "☀️🌙 mixto" y cuántos viajes hizo en cada uno. Mixto solo sale si el turno menor pesa al menos un 20%: un camión con 30 viajes de día y 1 de noche sigue siendo de día, si no toda la flota saldría mixta. LA VISTA DEL LISTERO NO CAMBIA: él sigue viendo el turno escrito en cada uno de sus viajes, y nada más.' },
      { t: 'note', text: '⏰ DE DÓNDE SALE EL TURNO. De la HORA del viaje, no de lo que se guardó el día que se registró. Son lo mismo en condiciones normales —al registrar, el sistema guarda el turno que corresponde a esa hora— pero deducirlo tiene dos ventajas: los viajes viejos, de antes de que se guardara el turno, ya no salen sin turno; y si a un viaje se le CORRIGE la hora cruzando las 7am o las 7pm, el turno se mueve con ella en vez de quedarse en el que tenía. Cuando lo guardado y la hora no concuerdan, la pantalla lo avisa en amarillo con la cantidad de viajes afectados y manda la hora.' },
      { t: 'note', text: '🔎 LOS FILTROS DE LA LISTA COMPLETA, REVISADOS (22/08/2026). Cuatro cosas cambiaron, y todas se notan. (1) EL NÚMERO DEL CHIP YA NO MIENTE: la cantidad entre paréntesis de cada empresa, listero y camión se cuenta ahora sobre los viajes que YA pasan los OTROS filtros marcados. Antes se contaba sobre el rango entero, así que con un listero marcado los chips de empresa seguían mostrando el total de TODOS los listeros y no cuadraban con la lista de abajo ni con el PDF. El chip de un eje NO se filtra a sí mismo, para poder seguir comparando y marcar un segundo. (2) UN FILTRO MARCADO YA NO SE ESCONDE: si cambias de día y el listero que tenías marcado no trabajó ese día, su chip sigue ahí, en (0). Antes desaparecía, la lista salía vacía y no había cómo saber qué la estaba tapando. (3) LA LISTA VACÍA AHORA DICE POR QUÉ: distingue "no hay viajes", "tienes marcado un filtro que no aparece en este rango", "esa combinación de filtros no da nada", "no marcaste ningún día" y "el rango está al revés". (4) DÍAS ESPECÍFICOS SIN NINGÚN DÍA MARCADO ya no muestra la jornada de hoy por su cuenta: lo dice y no muestra nada. Arriba de los filtros hay siempre una línea 📅 con el rango que está en efecto y cuántos viajes trae.' },
      { t: 'note', text: '📅 EL RANGO AL REVÉS Y EL PDF DE "DÍAS ESPECÍFICOS" (22/08/2026). Dos correcciones del reporte. Si en RANGO LIBRE el DESDE queda después del HASTA (en la web el campo de fecha deja escribir cualquier valor, y se puede borrar el HASTA), antes salían CERO viajes sin decir por qué y ese cero se podía exportar como si fuera un día flojo. Ahora no se consulta nada, se explica en rojo y el botón de exportar se niega. Y en DÍAS ESPECÍFICOS el encabezado del PDF ya NO dice "del 5 al 22": los días marcados no tienen por qué ser seguidos, y ese texto hacía leer 18 jornadas donde había 2. Ahora dice cuántas jornadas sueltas son y cuáles.' },
      ...
      { t: 'note', text: '🛡️ LA ÚLTIMA FUGA DE VIAJES, TAPADA (31/08/2026). Quedaba UN camino por el que un viaje desaparecía sin dejar rastro, y era el peor porque no daba ninguna señal. Pasaba así: un viaje que no puede subirse por un problema de datos (el camión se borró del catálogo, por ejemplo) se APARTA a la lista de rojos. Si justo en ese momento el teléfono no podía escribir en su memoria — memoria llena, navegador en modo privado, cookies bloqueadas — el error se ignoraba, y un instante después el viaje se sacaba de la cola por considerarlo ya resuelto. No quedaba ni en la cola ni en los apartados, y el aviso rojo tampoco salía. Ahora, si no se puede apartar, el viaje SE QUEDA EN LA COLA y el aviso lo dice. Se reintenta solo cuando el teléfono vuelve a poder guardar. La cola normal ya tenía esta protección desde el 22/08; a los apartados les faltaba.' },
