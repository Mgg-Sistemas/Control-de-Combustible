...
const SECTIONS: Sec[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '📊 Reporte por inspector — "Recorrido del inspector" (PDF): en Inspecciones → Reportes → ✅ Máquinas asignadas por inspector, con filtro de 📅 un día y de inspectores TIPO CHECK (marcas uno o varios; vacío = todos). Reconstruye el recorrido de cada inspector en orden cronológico: hora de la revisión, máquina, marca/modelo, serial/placa, sector/ubicación, estado en que la encontró y si estaba cerca (distancia GPS). Además trae, en el mismo documento, lo de un reporte diario: el TURNO de cada revisión, las horas de la máquina (día y noche) y las HORAS DE SU JORNADA — solo las del turno que esa persona cubrió, que es lo que cuenta para el pago —, más la columna "Inició" con el nombre de quien marcó el inicio de la jornada. Arriba sale un resumen general y cada inspector tiene su propio total.' },
      { t: 'note', text: '⚠️ Cómo leer las horas del Reporte por inspector: el total de cada inspector es SOLO el turno que cubrió. Una máquina puede trabajar de día y de noche el mismo día (el 16-ago-2026 le pasó a 102 de 173), y al inspector de día NO se le suman las horas de la noche: ese trabajo es del nocturno. El turno sale de la hora del check-in (día 7am–7pm). Por eso el total NO es la suma de las columnas "Máquina día" y "Máquina noche" — esas dos son el día completo de la máquina, para que veas de dónde sale la diferencia. Y nada se cuenta dos veces: aunque haya revisado la misma máquina 5 veces en su turno, esas horas suman UNA sola vez. Si una máquina no tiene ronda registrada ese día, sus horas salen "—" (no hay dato), que no es lo mismo que trabajar 0,00 horas. Se usa la MISMA fórmula que el Reporte del día por empresa y el Control de maquinaria, así que los números coinciden entre los tres. Este reporte agrupa por quién hizo el check-in de verdad (congelado en el momento), a diferencia del Histórico de Jornadas, que atribuye por la asignación actual y cambia cuando se reasignan máquinas.' },
