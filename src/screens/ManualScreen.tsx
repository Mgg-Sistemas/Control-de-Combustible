...
const SECTIONS: Sec[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: 'Máquina PARADA = avería automática: si en el check-in se marca la máquina como 🟡 Parada, se pide el MOTIVO (obligatorio) y se crea automáticamente una avería en Mantenimiento de Maquinaria. En Control de maquinaria esa máquina sale marcada como "🔴 MÁQUINA PARADA (avería)". El inspector asignado (último check-in) también sale en Control (🪖 Inspector). Al finalizar la jornada, las horas activas (horómetro final − inicial) pasan solas a Control de maquinaria en su turno (día/noche).' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '🔎 Buscar por tipo de equipo (en 📊 Conteo de equipos): dentro de la vista previa del reporte "Conteo de equipos" hay un buscador CON CASILLAS. Escribe el tipo (por ejemplo "volqueta toronto") para filtrar la lista y TILDA uno o varios tipos; abajo aparece un NÚMERO grande con el TOTAL de equipos y el desglose por empresa. Botón "⬇️ PDF de este conteo" para imprimir el total, la cantidad por tipo y por empresa.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'note', text: '✅ CHECK MÁQUINA (asignación por TURNO): cada máquina tiene DOS inspectores: uno de ☀️ Día y otro de 🌙 Noche. Para asignarte, toca ✅ CHECK MÁQUINA (debajo del escáner), busca la máquina por nombre/serial/placa y toca el TURNO que cubres (☀️ Día o 🌙 Noche). Si el turno ya lo tiene otro inspector, sale su nombre; al tocarlo lo reemplazas. Toca de nuevo tu turno para quitarte. Cada inspector solo ve en su ronda las máquinas donde es de día o de noche, e indica "Mi turno". Queda en la Auditoría (✅ se asignó · Día/Noche). El administrador tiene además "Ver todas".' },
      { t: 'p', text: 'Cómo marca el inspector una máquina:' },
      { t: 'steps', items: [
        'Entra con su usuario y contraseña (desde teléfono, cualquiera cae aquí). Ve "Mis máquinas asignadas".',
        'ASIGNAR: si la lista está vacía, toca ✅ CHECK MÁQUINA y asígnate las que inspeccionas. También puedes escanear el QR directo con el botón 📷.',
      ...
      { t: 'note', text: '✅ Máquinas asignadas por inspector (CHECK): arriba del módulo de Inspecciones sale la lista de qué máquinas se asignó cada inspector con el botón ✅ CHECK MÁQUINA del teléfono. Se sincroniza EN VIVO. En pantalla ves inspector → sus máquinas con sector, referencia, serial/placa, empresa y fecha/hora.' },
      { t: 'note', text: 'Dos reportes en Inspecciones: (1) 📊 Reporte por inspector (día/rango, con hora, sector, serial/placa, empresa y ESTADO de la máquina); y (2) 📄 Asignaciones por sector (las máquinas que cada inspector se asignó con el CHECK, agrupadas POR SECTOR, con TURNO ☀️/🌙 + referencia + serial + placa + empresa, SIN el estado). El reporte (2) tiene BÚSQUEDA libre (máquina, placa, serial, empresa, inspector, encargado, edificio) y filtros tipo CHECK por INSPECTOR y por EDIFICIO/referencia (con su propia búsqueda).' },
