...
export async function generateChoferCombustibleGuide(): Promise<boolean> {
  ...
  const step3 = stepSection(3, 'Registra el surtido', 'Sector y Parroquia son solo para confirmar que estás en el lugar correcto',
    twoCol(
      stepList([
        '**Sector** y **Parroquia** salen ya llenos, solo lectura — sirven para confirmar que estás en la máquina correcta.',
        'Escribe los **Litros surtidos** y el **Monto por litro**.',
        'El **Monto total** se calcula solo (litros × monto por litro), no se edita.',
        'Toca 📷 **Agregar foto** las veces que necesites; cada miniatura tiene una **×** para borrarla.',
        'Toca **Registrar surtido** para guardar.',
      ]) +
      calloutInfo('<b>Sin tope diario:</b> a diferencia de otros roles, el chofer de combustible <b>no tiene</b> el límite de "2× el consumo esperado" — puede surtir sin ese tope.') +
      calloutWarn('Después de guardar, el formulario se queda <b>abierto</b> en la misma máquina (litros, monto y fotos se limpian) para registrar un <b>segundo surtido</b> sin volver a buscarla. Toca <b>Cerrar</b> solo cuando termines con esa máquina.'),
      step3Mock
    )
  );
  ...
  const summary = summaryPage({
    ...
    rows: [
      ...
      ['Vas a surtir', 'Llena **Litros surtidos** y **Monto por litro**, agrega fotos si quieres y toca **Registrar surtido**.'],
      ['Debes surtir otra vez a la misma máquina', 'El formulario queda **abierto y limpio**: solo llena los datos y **Registrar surtido** de nuevo.'],
