...
export async function cargarDatosUbicaciones(desde: string, hasta: string): Promise<DatosUbicaciones> {
  ...
  const [maqs, pts, vis, ron, cam] = await Promise.all([
    selectAllRows('machinery', 'id, code, marca, modelo, plate, serial, clasificacion, referencia, sector, latitude, longitude, company:company_id(name)'),
  ...
  const maquinas: MaquinaUbic[] = (maqs as any[]).map((m) => ({
    ...
    // `machinery.sector` es texto escrito a mano: casi siempre «Este»/«Oeste», a veces
    // el nombre de una obra. Se pasa crudo y `zonaCardinal` decide si sirve de cardinal.
    cardinalFicha: limpio(m.sector),
